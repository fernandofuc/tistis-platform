...
export function StationFormModal({
  ...
  // Track if user has modified form (for unsaved changes warning)
  const [isDirty, setIsDirty] = useState(false);
  // Track if auto-generation has happened for current session
  const [hasAutoGenerated, setHasAutoGenerated] = useState(false);

  // Initialize form when station changes or modal opens
  useEffect(() => {
    if (station) {
  ...
  // Auto-generate code from station type (only once per session, only for new stations)
  useEffect(() => {
    // Only auto-generate for new stations, when modal just opened, and hasn't auto-generated yet
    if (!isEdit && stationType && !hasAutoGenerated && isOpen) {
  ...
  }, [stationType, isEdit, existingCodes, hasAutoGenerated, isOpen]);
  ...
  const validate = (): boolean => {
    ...
    if (printerIp) {
      ...
      const match = printerIp.match(ipRegex);
      if (!match) {
        newErrors.printerIp = 'IP inválida (ej: 192.168.1.100)';
      } else {
        const octets = [match[1], match[2], match[3], match[4]].map(Number);
        const invalidOctet = octets.some(octet => octet > 255);
        if (invalidOctet) {
          newErrors.printerIp = 'Cada número de la IP debe estar entre 0 y 255';
        }
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : 'Error al guardar la estación');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleClose = () => {
    if (isDirty && !loading) {
      const confirmClose = window.confirm('¿Descartar los cambios sin guardar?');
    ...
    >
      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <h2 id="station-modal-title" className="text-xl font-bold text-slate-900">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
          <div className="p-6 space-y-6">
            ...
            <StationTypeSelector
              ...
              onChange={handleFieldChange(setStationType)}
            />
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormInput
                ...
                id="code"
                value={code}
                onChange={handleFieldChange(setCode)}
            ...
            <ColorPicker
              ...
              value={displayColor}
              onChange={handleFieldChange(setDisplayColor)}
            />
            ...
            <div className="bg-slate-50 rounded-xl p-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormInput
                  ...
                  id="printerName"
                  value={printerName}
                  onChange={handleFieldChange(setPrinterName)}
                ...
                <FormInput
                  ...
                  id="printerIp"
                  value={printerIp}
                  onChange={handleFieldChange(setPrinterIp)}
            ...
            <div className="bg-slate-50 rounded-xl p-4">
              <ToggleSwitch
                ...
                checked={isActive}
                onChange={handleFieldChange(setIsActive)}
              />
            </div>
          </div>
