-- =====================================================
-- TIS TIS PLATFORM - Fix RPC Column Names
-- Migration 131: Use correct column names from actual tables
-- =====================================================
-- PROBLEMA: El RPC get_tenant_ai_context usaba nombres de
-- columnas que no existen en las tablas reales:
--
-- ai_scoring_rules:
--   - Usaba: keywords      → Real: detection_config (JSONB)
--   - Usaba: category      → Real: category (OK)
--
-- ai_competitor_handling:
--   - Usaba: aliases       → Real: competitor_aliases
--   - Usaba: strategy      → Real: response_strategy
--
-- ai_response_templates:
--   - Usaba: trigger_phrase → Real: trigger_type
--   - Usaba: variables      → NO EXISTE
--
-- ai_business_policies:
--   - Usaba: branch_id     → NO EXISTE (no tiene branch)
--
-- SOLUCIÓN: Recrear RPC con columnas correctas
-- =====================================================
...
BEGIN
    ...
    -- COLUMNAS REALES: signal_name, points, detection_config, category
    -- =========================================
    ...
    -- COLUMNAS REALES: instruction_type, title, instruction, examples, branch_id, priority
    -- =========================================
    ...
    -- COLUMNAS REALES: policy_type, title, policy_text, short_version
    -- NOTA: NO tiene branch_id
    -- =========================================
    ...
    -- COLUMNAS REALES: category, title, content, summary, branch_id
    -- =========================================
    ...
    -- COLUMNAS REALES: trigger_type, name, template_text
    -- NOTA: NO tiene trigger_phrase ni variables
    -- =========================================
    ...
    -- COLUMNAS REALES: competitor_name, competitor_aliases, response_strategy,
    --                  talking_points, avoid_saying
    -- =========================================
...
DO $$
BEGIN
    RAISE NOTICE 'Migration 131 completed: Fixed RPC column names to match actual table schemas';
    RAISE NOTICE 'Column fixes:';
    RAISE NOTICE '  - ai_scoring_rules: keywords → detection_config->keywords';
    RAISE NOTICE '  - ai_competitor_handling: aliases → competitor_aliases, strategy → response_strategy';
    RAISE NOTICE '  - ai_response_templates: trigger_phrase → trigger_type, removed variables';
    RAISE NOTICE '  - ai_business_policies: removed branch_id (not in schema)';
END $$;
