-- =====================================================
-- Migration 032: FINAL RLS FIX
-- =====================================================
-- Orden correcto: 1) Drop policies, 2) Drop function, 3) Create new
-- =====================================================
...
-- CLINICAL_HISTORY
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'clinical_history') THEN
...
-- PATIENT_FILES
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'patient_files') THEN
...
-- =====================================================
-- PASO 2: AHORA SÍ ELIMINAR LAS FUNCIONES
-- =====================================================
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'messages') THEN
    EXECUTE 'CREATE POLICY "m1" ON public.messages FOR ALL USING (tenant_id = public.get_tenant())';
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'quote_items') THEN
    EXECUTE 'CREATE POLICY "qi1" ON public.quote_items FOR ALL USING (EXISTS (SELECT 1 FROM public.quotes q WHERE q.id = quote_items.quote_id AND q.tenant_id = public.get_tenant()))';
